...
import { ArrowRight, BookOpen, Box, CheckSquare, ChevronDown, ChevronRight, Copy, Download, FileText, FilePlus2, Folder, FolderInput, FolderOpen, FolderPlus, GalleryHorizontal, ListTree, Loader2, Network, PanelLeft, Plus, Search, Send, Settings, Trash2, X, Eye, Pencil, Link2, MoreHorizontal, Quote, Table2, MoveRight } from 'lucide-react';
...
export default function Workspace() {
  ...
  const visible = useMemo(() => entries.filter(entry => (view === 'notes' ? entry.kind === 'note' : entry.kind === 'agent') && (!query || entry.title.toLowerCase().includes(query.toLowerCase()))), [entries, query, view]);
  ...
  function switchView(next: 'notes' | 'sessions' | 'anki') {
    ...
    if (next !== 'anki' && selected?.kind !== (next === 'notes' ? 'note' : 'agent')) {
      setSelected(null);
      setSelectedId(null);
    }
  }
  ...
  return <div className={`obsidian-app${sidebarOpen ? '' : ' sidebar-collapsed'}${railOpen ? '' : ' rail-collapsed'}`} style={{ gridTemplateColumns: `${sidebarOpen ? sidebarWidth : 0}px minmax(0, 1fr) ${railOpen ? railWidth : 0}px` }}>
    <motion.aside ref={sidebarRef} className="obsidian-sidebar" aria-hidden={!sidebarOpen} inert={!sidebarOpen}
      ...
      <div className="sidebar-activity" aria-label="工作区导航"><div className="activity-main">{([
        { id: 'notes', label: '笔记', Icon: Folder },
        { id: 'sessions', label: '会话', Icon: Box },
        { id: 'anki', label: '闪卡', Icon: GalleryHorizontal },
      ] as const).map(({ id, label, Icon }) => <motion.button key={id} className={view === id ? 'is-active' : ''} type="button" title={label} aria-label={label} aria-current={view === id ? 'page' : undefined} whileTap={reduceMotion ? undefined : { scale: 0.94 }} onClick={() => switchView(id)}>{view === id ? <motion.span className="activity-current-mark" layoutId="workspace-active-view" transition={{ duration: reduceMotion ? 0 : 0.2, ease: [0.22, 1, 0.36, 1] }} /> : null}<Icon size={19} /><span>{id === 'notes' ? '文件' : label}</span></motion.button>)}</div><motion.button className="activity-settings" type="button" title="知识图谱" aria-label="知识图谱" whileTap={reduceMotion ? undefined : { scale: 0.94 }} onClick={() => setGraphOpen(true)}><Network size={19} /><span>图谱</span></motion.button></div>
      <div className="sidebar-pane">
      <div className="sidebar-pane-header"><span>{view === 'notes' ? '文件' : view === 'sessions' ? '会话' : '闪卡'}</span><button type="button" title="收起侧栏" aria-label="收起侧栏" onClick={() => setSidebarOpen(false)}><PanelLeft size={16} /></button></div>
      <label className="obsidian-search"><Search size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索笔记..." /></label>
      <div className="tree-heading"><span>{view === 'notes' ? '笔记库' : view === 'sessions' ? '智能体会话' : '记忆卡片'}</span>{view === 'notes' ? <div className="tree-heading-actions"><button type="button" onClick={() => void createNote()} aria-label="新建笔记"><Plus size={15} /></button><button type="button" onClick={() => createFolder()} aria-label="新建文件夹"><FolderPlus size={15} /></button></div> : view === 'anki' ? <button type="button" onClick={() => saveAnkiCards([...ankiCards, { id: crypto.randomUUID(), front: '', back: '', tags: '' }])} aria-label="新建卡片"><Plus size={15} /></button> : null}</div>
      <nav className="obsidian-tree">{view === 'anki' ? <div className="anki-sidebar-list">{ankiCards.map((card, index) => <button key={card.id} type="button" className="session-tree-item" onClick={() => { document.getElementById(`anki-card-${card.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }); if (window.innerWidth <= 720) setSidebarOpen(false); }}><GalleryHorizontal size={15} /><span>{card.front || `新卡片 ${index + 1}`}</span></button>)}</div> : view === 'notes' ? <>{folderRoots.map(renderFolder)}{roots.map(entry => <TreeItem key={entry.id} entry={entry} entries={visible} selectedId={selectedId} onSelect={openEntry} onContextMenu={openContextMenu} />)}</> : roots.map(entry => <button key={entry.id} type="button" className={`session-tree-item${selectedId === entry.id ? ' is-active' : ''}`} onClick={() => void openEntry(entry.id)}><Box size={15} /><span>{entry.title}</span></button>)}</nav>
    ...
    <main className="obsidian-main" inert={(sidebarOpen || railOpen) && window.innerWidth <= 720}>
      <header className="obsidian-topbar"><Button variant="ghost" size="icon" className="sidebar-opener" onClick={() => setSidebarOpen(value => !value)} aria-label={sidebarOpen ? '收起侧栏' : '打开侧栏'}><PanelLeft size={18} /></Button><div className="workspace-tab"><span>{view === 'anki' ? '闪卡' : selected?.title || (view === 'sessions' ? '会话' : '新建笔记')}</span></div><div className="topbar-actions"><Button variant="ghost" size="icon" onClick={() => { setView('notes'); setSelected(null); setSelectedId(null); setRailOpen(false); }} aria-label="打开新建笔记页" title="打开新建笔记页"><Plus size={18} /></Button><Button variant="ghost" size="icon" onClick={() => setCommandOpen(true)} aria-label="快速切换" title="快速切换"><Search size={17} /></Button><Button variant="ghost" size="icon" onClick={() => setRailOpen(value => !value)} aria-label="切换信息栏" title="切换信息栏"><PanelLeft size={17} /></Button></div><div className="mobile-topbar-actions">{selected?.kind === 'note' ? <button type="button" onClick={() => setEditorMode(value => value === 'edit' ? 'preview' : 'edit')} aria-label={editorMode === 'edit' ? '阅读模式' : '编辑模式'}>{editorMode === 'edit' ? <BookOpen size={18} /> : <Pencil size={18} />}</button> : null}<button type="button" onClick={event => { event.stopPropagation(); if (selected?.kind === 'note') setContextMenu({ x: 0, y: 0, kind: 'note', id: selected.id }); else setCommandOpen(true); }} aria-label="更多操作"><MoreHorizontal size={19} /></button></div></header>
      {error ? <div className="workspace-error">{error}<button type="button" onClick={() => setError('')}><X size={14} /></button></div> : null}
      {view === 'anki' ? <section className="anki-editor"><div className="anki-header"><div><h1>闪卡</h1><p>兼容 Anki TSV 导入格式</p></div><Button onClick={exportAnki} disabled={!ankiCards.length}><Download size={16} /> 导出 Anki</Button></div><div className="anki-cards">{ankiCards.map(card => <div className="anki-card" id={`anki-card-${card.id}`} key={card.id}><div className="anki-card-head"><span>卡片</span><button type="button" onClick={() => saveAnkiCards(ankiCards.filter(item => item.id !== card.id))}><X size={14} /></button></div><label>正面<textarea value={card.front} onChange={event => saveAnkiCards(ankiCards.map(item => item.id === card.id ? { ...item, front: event.target.value } : item))} placeholder="问题或提示" /></label><label>背面<textarea value={card.back} onChange={event => saveAnkiCards(ankiCards.map(item => item.id === card.id ? { ...item, back: event.target.value } : item))} placeholder="答案、解释或例子" /></label><label>标签<input value={card.tags} onChange={event => saveAnkiCards(ankiCards.map(item => item.id === card.id ? { ...item, tags: event.target.value } : item))} placeholder="例如：高数 期末" /></label></div>)}</div>{!ankiCards.length ? <div className="anki-empty"><GalleryHorizontal size={25} /><h2>还没有闪卡</h2><p>点击左侧 +，建立第一张记忆卡片。</p></div> : null}</section> : view === 'sessions' && !selected ? <section className="workspace-blank"><Box size={25} /><p>从左侧选择会话</p></section> : !selected ? <NewNoteHome entries={entries} onCreate={(noteTitle, initialBody) => void createNote(noteTitle, initialBody)} onOpen={id => void openEntry(id)} /> : selected.kind === 'agent' ? <section className="session-view"><div className="session-heading"><div className="session-icon"><Box size={20} /></div><div><p>会话</p><h1>{selected.title}</h1></div></div><div className="chat-messages">{(chat?.messages ?? []).filter(message => message.content).map((message, index) => <div key={`${message.created_at}-${index}`} className={`chat-message ${message.role}`}><small>{message.role === 'user' ? '你' : 'TJUClaw'}</small><p>{message.content}</p></div>)}</div><form className="chat-composer" onSubmit={handleChat}><textarea value={draft} onChange={event => setDraft(event.target.value)} placeholder="输入消息，按 Enter 发送..." rows={1} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} /><button type="submit" aria-label="发送" disabled={!draft.trim()}><Send size={17} /></button></form></section> : <article className="note-editor" onContextMenu={event => openContextMenu(event, 'editor')}><div className="note-toolbar"><div className="mode-switch"><button type="button" className={editorMode === 'edit' ? 'is-active' : ''} onClick={() => setEditorMode('edit')} aria-label="编辑模式" title="编辑模式"><Pencil size={16} /></button><button type="button" className={editorMode === 'preview' ? 'is-active' : ''} onClick={() => setEditorMode('preview')} aria-label="阅读模式" title="阅读模式"><Eye size={16} /></button></div></div><input ref={titleRef} className="note-title" value={title} onChange={event => { setTitle(event.target.value); queueSave(event.target.value, body); }} placeholder="未命名笔记" />{editorMode === 'preview' ? <div className="markdown-preview" dangerouslySetInnerHTML={{ __html: renderMarkdown(body) }} /> : <MarkdownEditor key={selected.id} value={body} onChange={nextBody => { setBody(nextBody); queueSave(title, nextBody); }} editorRef={bodyRef} />}</article>}
      <nav className="mobile-command-bar" aria-label="快捷操作"><button type="button" onClick={() => setSidebarOpen(true)} aria-label="打开文件"><Folder size={19} /></button><button type="button" onClick={() => setCommandOpen(true)} aria-label="搜索和快速切换"><Search size={19} /></button><button type="button" onClick={() => void createNote()} aria-label="新建笔记"><Plus size={22} /></button><button type="button" onClick={() => { setSidebarOpen(true); switchView('sessions'); }} aria-label="打开会话"><Box size={19} /></button><button type="button" onClick={() => { setSidebarOpen(true); switchView('anki'); }} aria-label="打开闪卡"><GalleryHorizontal size={19} /></button></nav>
    ...
    <Dialog open={commandOpen} onOpenChange={setCommandOpen}><DialogContent className="command-dialog"><DialogTitle>快速切换</DialogTitle><DialogDescription>跳转到笔记、会话或工具。</DialogDescription><div className="command-list">{entries.filter(item => item.title.toLowerCase().includes(query.toLowerCase())).slice(0, 12).map(item => <button key={item.id} type="button" onClick={() => { setView(item.kind === 'agent' ? 'sessions' : 'notes'); void openEntry(item.id); setCommandOpen(false); }}><Link2 size={15} /><span>{item.title}</span><small>{item.kind === 'agent' ? '会话' : '笔记'}</small></button>)}<button type="button" onClick={() => { setView('anki'); setCommandOpen(false); }}><GalleryHorizontal size={15} /><span>打开闪卡</span><small>工具</small></button></div></DialogContent></Dialog>
  </div>;
}
