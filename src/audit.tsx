...
export default function Audit() {
  ...
  const [mode, setMode] = useState(location.hash.startsWith('#GB-') ? 'compare' : 'auth');
  ...
  return <div className="audit">
    ...
    <main className="desk">
      ...
      <section className="intro"><div><p>看清差异，再推进实现。</p><h1>{mode === 'auth' ? '从邮箱输入，到安全退出。' : '每一个状态，都有据可对。'}</h1></div>{mode !== 'auth' && <div className="coverage"><code>{screens.filter(s => s.captures[viewport]).length} / {screens.length}</code><p>{vp.label}已关联截图 · 不等于验收通过</p></div>}</section>
      <div className="tabs">{[['auth', '邮箱登录'], ['compare', '截图对照'], ['flow', '参考线路'], ['gallery', '全部参考']].map(([key, label]) => <button key={key} aria-pressed={mode === key} onClick={() => setMode(key)}>{label}</button>)}<span>{filtered.length} 个匹配页面</span></div>
      {error ? <section className="empty-state" role="alert"><h2>还没有读到证据</h2><p>{error}</p></section> : !current ? <section className="empty-state" role="status">正在加载证据…</section> : <>
        {mode === 'auth' && <AuthAudit screens={screens} refresh={refresh} onZoom={setZoom} onReference={screen => { choose(screen); setMode('compare'); }} />}
